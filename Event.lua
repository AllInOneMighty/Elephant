  ...
  -- Not channel messages
    ...
    if prat_msg then
      ...
      if event == "PARTY_LOOT_METHOD_CHANGED" then
        local method, masterloot_party, masterloot_raid
        if Elephant.GetLootMethodCompat then
          method, masterloot_party, masterloot_raid = Elephant:GetLootMethodCompat()
        end
        if not method and type(GetLootMethod) == "function" then
          method, masterloot_party, masterloot_raid = GetLootMethod()
        end
        ...
        if method ~= Elephant.tempConf.lootmethod then
          ...
          msg.arg1 = Elephant.L['lootmethod'][method] or tostring(method)
        else
