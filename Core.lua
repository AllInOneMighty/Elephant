...
--[[ Compatibility: Blizzard has been cleaning the global API namespace in some Classic clients.
     GetLootMethod() may not exist anymore; it has moved to C_PartyInfo.GetLootMethod().
     This helper keeps Elephant's old string-based loot method behavior. ]]
function Elephant:GetLootMethodCompat()
  if type(GetLootMethod) == "function" then
    return GetLootMethod()
  end

  if C_PartyInfo and type(C_PartyInfo.GetLootMethod) == "function" then
    local method, masterloot_party, masterloot_raid = C_PartyInfo.GetLootMethod()

    -- C_PartyInfo returns an enum (number). Elephant historically used strings.
    local map = {
      [0] = "freeforall",
      [1] = "roundrobin",
      [2] = "master",
    ...
    return map[method] or method, masterloot_party, masterloot_raid
  end
