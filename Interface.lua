...
--[[
  ...
  Note that if an existing color string uses the same color as the given values,
  it will not be detected and will be treated like the other ones.
]]
...
--[[
  ...
  line to the last one of the log, and finally shows the log.
]]
...
--[[
  ...
  title, the current line and the frame itself to the new log color, changes the
  name of log text, updates the current line information, and updates the status
  of log buttons.
...
--[[
  ...
  you wish to scroll up in the log. This method checks if doing so gets the user
  out of the current log (before first line or after last line), and in that
  case forces him/her back to the corresponding limit, so you don't have to do
  it yourself.
]]
...
--[[
  Displays or hides the Elephant button, depending on its current status. If the
...
--[[
  Returns color value to use when something is enabled or disabled. When calling
  this method, you need to specify which color you wish to get the value: "r",
  "g" or "b".
]]
